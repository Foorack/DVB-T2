...
public class Parity_Interleaver extends Pipeline<FEC_Frame, FEC_Frame> {
	...
	private static class Bit_Interleave extends Filter<FEC_Frame, FEC_Frame> {
		...
		public void work() {
			...
			for (int i = 0; i < K_ldpc; i++) {
				D_out[i] = D_in[(int) (i)]; // Information bits are not
											// interleaved
			}

			for (long t = 0; t < Q_ldpc; t++) {
				for (long s = 0; s < 360; s++) {
					D_out[(int) (K_ldpc + 360 * t + s)] = D_in[(int) (K_ldpc + Q_ldpc * s + t)]; // parity
																									// interleaving
