...
public class P2_Symbol {
	...
	static BitSet getL1PreSignal() {
		...
		// L1_REPITITION_FLAG 1 bit: 0 -> dynamic signaling shall not be
		// provided for the next frame within this frame
		index++;
		...
		index += 16;
		// NUM_T2_FRAMES 8 bit: 00000010 -> 2 T2 frames per superframe (min is 2
		// frames)
		index += 7;
		L1_pre_signalling.set(index);
		P2.set(index);
	...
	static BitSet getL1PostSymbol() {
		...
		index += 15;
		L1_post_signalling.set(index);
		P2.set(preSignalLength + index);
		...
		index += 10;
		// FRAME_INTERVAL 8bit: 1 -> T2 frame interval of the current PLP for
		// within the superframe. Appear in every frame
		index += 8;
		L1_post_signalling.set(index);
		P2.set(preSignalLength + index);

		/*
		 * TIME_IL_LENGTH and TIME_IL_TYPE shall only both be set to '0' when
		 * NUM_PLP is set to '1'.
		 */
		// TIME-IL_LENGTH 8bit: 00000000 -> Time interleaving not used for
		// current PLP
		index += 8;
		// TIME_IL_TYPE 1bit: 0
		index++;
		...
		// STATIC_FLAG 1bit : 1 -> scheduling for the current PLP varies from T2
		// frame to T2 frame(0) or remain static (1)
		index++;
		 ...
		 * PLP looping is over
		 */
		// FEF_LENGTH_MSB 2bit: -> 2 MSB of FEF_LENGTH for future use. but NOT
		// for bias balancing
		index += 2;
		 ...
		 * Dynamic field
		 */
		// FRAME_IDX 8bit : -> index of the current T2 frame within a super
		// frame
		index += 8;
		...
		index += 22;
		// L1_CHANGE_COUNTER 8bit: 0 -> number of super frames ahead of config
		// change has. 0 for no future change foreseen
		index += 8;
		 ...
		 * PLP looping fields
		 */
		// PLP_ID 8bit: same as the order within the PLP loop in the L1-post
		...
		/**
		 * L1 post extension field is not present since L1_POST_EXTENSION is set
		 * to 0
		 */
