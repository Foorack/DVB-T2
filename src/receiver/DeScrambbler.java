...
public class DeScrambbler extends Pipeline<FEC_Frame, FEC_Frame> {
	...
	public DeScrambbler() {
		...
		this.add(new Splitjoin<FEC_Frame, FEC_Frame>(new RoundrobinSplitter<FEC_Frame>(1),
				new RoundrobinJoiner<FEC_Frame>(1), new de_scrambble(), new de_scrambble()
	...
	private static class de_scrambble extends Filter<FEC_Frame, FEC_Frame> {
		...
		public void work() {
			...
			boolean[] initialdata = new boolean[] { true, false, false, true, false, true, false, true, false, false,
					false, false, false, false, false };
			for (int i = 0; i < inputstream.length; i++) {
				if (counter > BBFRAMElength) {
					counter = 0;
					initialdata = new boolean[] { true, false, false, true, false, true, false, true, false, false,
							false, false, false, false, false };
				}
				if (counter <= BBFRAMElength) {
