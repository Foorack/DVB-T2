 ...
 */
public class Maximum_Likelyhood_Mapper extends Pipeline<Complex, Complex> {

	@SuppressWarnings("unchecked")
	public Maximum_Likelyhood_Mapper() {
		// this.add(new ML_Map());
		this.add(new Splitjoin<Complex, Complex>(new RoundrobinSplitter<Complex>(8100),
				new RoundrobinJoiner<Complex>(8100), new ML_Map(), new ML_Map(), new ML_Map(), new ML_Map()
	...
	private static class ML_Map extends Filter<Complex, Complex> {
		...
		@Override
		public void work() {
			// System.out.println("ML Mapper work
			// method-----------------------");
			...
			for (int i = 0; i < 8100; i++) { // 8100
				...
				// real == De_Normalizer_Out[i].getReal()+1)){
				if (real % 2 == 0 && real == De_Normalizer_Out[i].getReal()) {
				...
				// || imge == De_Normalizer_Out[i].getImaginary()+1)){
				if (imge % 2 == 0 && imge == De_Normalizer_Out[i].getImaginary()) {
					// int min = -9;
					// int max = 8;
					//
		...
		private static Complex[] Maximum_Likelyhood_Out(Complex[] De_Normalizer_Out) {
			...
			for (int i = 0; i < 6; i++) {
				...
				if (real % 2 == 0
						&& (real == De_Normalizer_Out[i].getReal() || real == De_Normalizer_Out[i].getReal() + 1)) {
					int min = -9;
					int max = 8;
