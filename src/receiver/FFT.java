 ...
 */
public class FFT extends Pipeline<Complex, Complex> {

	@SuppressWarnings("unchecked")
	public FFT() {
		// this.add(new dofft());
		this.add(new Splitjoin<Complex, Complex>(new RoundrobinSplitter<Complex>(64), new RoundrobinJoiner<Complex>(64),
				new dofft(), new dofft()

		));
	}
	/*
	 * public static void main(String[] args) { int N = 8; Complex[] x = new
	 * Complex[N]; // original data for (int i = 0; i < N; i++) { x[i] =
	 * Complex.valueOf(i, 0); x[i] = Complex.valueOf(-2*Math.random() + 1, 0); }
	 * show(x, "x");
	 * 
	 * // FFT of original data Complex[] y = fft(x); show(y, "y = fft(x)");
	 * 
	 * }
	 */
