...
public class Bit_DeInterleaver extends Pipeline<FEC_Frame, FEC_Frame> {
	...
	public Bit_DeInterleaver() {
		...
		this.add(new Splitjoin<FEC_Frame, FEC_Frame>(new RoundrobinSplitter<FEC_Frame>(1),
				new RoundrobinJoiner<FEC_Frame>(1), new Bit_Deinterleave(), new Bit_Deinterleave()
	...
	private static class Bit_Deinterleave extends edu.mit.streamjit.api.Filter<FEC_Frame, FEC_Frame> {
		...
		public void work() {
			...
			for (int t = 0; t < Q_ldpc; t++) {
				for (int s = 0; s < 360; s++) {
					Deint[K_ldpc + Q_ldpc * s + t] = D_out[K_ldpc + 360 * t + s]; // parity
																					// de
																					// interleaving
				}
			}
			...
			FEC_Frame interleaved_frame = new FEC_Frame(Deint);
