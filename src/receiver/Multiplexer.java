 ...
 */
public class Multiplexer extends Pipeline<Eightout, FEC_Frame> {

	@SuppressWarnings("unchecked")
	public Multiplexer() {
		// this.add(new do_Multiplexing());
		this.add(new Splitjoin<Eightout, FEC_Frame>(new RoundrobinSplitter<Eightout>(1),
				new RoundrobinJoiner<FEC_Frame>(1), new do_Multiplexing(), new do_Multiplexing()
	...
	private static class do_Multiplexing extends Filter<Eightout, FEC_Frame> {
		...
		public void work() {
			...
			/*
			 * boolean temp2; System.out.println("Mux Array 0--------------");
			 * for (int i = 0; i < 1000; i++) { temp2 = array0[i]; if(temp2){
			...
			/*
			 * for(int i=0; i<4050; i++){ for(int j=0; j<16; j++){
			...
			///////////////////////////////////
			// System.out.println("Multiplexer stream out size =
			// "+stream.length);
			...
			// =====================================================
			/*
			 * boolean temp; System.out.println("outputstream--------------");
			 * for (int i = 0; i < 1000; i++) { temp = shiftedstream[i];
			 * if(temp){ System.out.print("1"); }else { System.out.print("0"); }
			 * } System.out.println();
			 */
			...
			push(out);
			/*
			 * for (int i = 0; i < streamout.length; i++) { if (streamout[i]) {
			 * push((byte) 1); }else { push((byte) 0); } }
			 */
			//////////////////////////////////////////////
			/*
			 * Random rand = new Random();
			 * 
			 * for (int i = 0; i < 64800; i++) { int x = rand.nextInt(2); if (x
			 * == 1) { push((byte) 1); }else { push((byte) 0); } }
			 */
			/////////////////////////////////////////////
		}
