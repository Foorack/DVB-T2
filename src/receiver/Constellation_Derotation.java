 ...
 */
public class Constellation_Derotation extends Pipeline<Complex, Complex> {
	...
	public Constellation_Derotation() {
		...
		this.add(new Splitjoin<Complex, Complex>(new RoundrobinSplitter<Complex>(8100),
				new RoundrobinJoiner<Complex>(8100), new DeRotator(), new DeRotator()

		));
	}

	private static class DeRotator extends Filter<Complex, Complex> {
		...
		public void work() {
			...
			///////////////////////////////////////////////////////////
			double angle = Math.atan(0.0625) * 180 * (1 / Math.PI); // atan
																	// value
																	// should be
																	// in
																	// degrees
			...
			// System.out.println("RQD =
			// "+RQD.getReal()+"\t"+RQD.getImaginary()+"\n");
