...
public class PushDataBits extends Pipeline<FEC_Frame, Byte> {
	...
	private static class DataPush extends Filter<FEC_Frame, Byte> {
		...
		public void work() {
			...
			// System.out.println("data bits from FEC:
			// "+builder.toString().substring(0, 400));
			// appendToFile(builder.toString());
		}

		private static void appendToFile(String text) {
			try (PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("final_output.txt", true)))) {
